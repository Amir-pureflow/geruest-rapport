...
## Feedback Bauführer 20.09. (Demo durch Erin) — umgesetzt, ausser Sprachen

Quelle: `../Feedback_Bauführer_Prototyp.docx` (elf WhatsApp-Punkte). **Punkt A (dreizehn Sprachen) ist bewusst
noch offen** — kommt mit dem Transkriptions-Spike (Whisper/Google/Anthropic); `mitarbeiter.sprache` kennt weiter nur de/ar/pl/en.

- **Zeiten von–bis** (B–D): In der Erfassung schaltet man fürs Team oder je Person zwischen «Stundenzahl» (wie bisher,
  Vorgabe 8.4 h) und «von – bis» um. Bis zu zwei Spannen (Vormittag, Nachmittag), native Uhrzeit-Wahl, Vorgabe 7:00 mit
  offenem «bis» — **nichts wird geraten**, ohne «bis» lässt sich nicht speichern. **Die App rechnet keine Pausen**: 7:00–16:00
  sind 9.0 h; der Mittag ist die Lücke zwischen den zwei Spannen. Summe → Normal (bis 8.4 h) + Überstunden wie auf dem
  Wochenblatt. Spalten `zeiteintrag.von_min/bis_min/von2_min/bis2_min` (Migration 0016, Minuten seit Mitternacht), nur
  gesetzt, wenn Zeiten eingetragen wurden; Cockpit und «Meine Woche» zeigen sie. Logik in `src/lib/zeiten.ts` (mit Tests).
  **Offen mit Arbnor:** Mittag fest oder frei, bezahlt oder nicht; weitere Pausen; trägt der Chefmonteur für alle ein.
- **Sprachnotiz immer** (1): Die Aufnahme ist die «Bemerkung zum Tag» — immer sichtbar, freiwillig; bei Überstunden
  weiterhin Pflicht. Das Cockpit zeigt Notizen ohne Regieverdacht als eigene Karte «Bemerkung zum Tag».
- **Person hinzufügen** (2): «Person hinzufügen» in der Erfassung — aus der Mitarbeiterliste (nie frei eingetippt, Regel 2
  bleibt), Chefmonteur-Ausnahme wie bei der Konto-Nr.: Name ab 2 Buchstaben, max. 5 Treffer, dazu «Zuletzt dabei» je Team
  (`localStorage teamgeraet-gaeste-<teamId>`). Gäste bekommen ein «Gast»-Schild und landen nur im `zeiteintrag`, nicht in
  `team_mitglied`.
- **Nur Auto** (3): `OEV_AKTIV = false` in Erfassung.tsx — öV-Chip ausgeblendet, `oev` bleibt im Datenmodell (Export zeigt
  alte Werte weiter). Ob öV ganz weg soll: klären, dann Spalte und `oev_standard` aufräumen.
- **Temporärbüro-Export** (4): Sekretariat → Export → Abschnitt «Temporärbüros»: je Büro ein eigenes Excel mit nur dessen
  Leuten (`excelBuero`, Datei `Temporaer_<Büro>_<Zeitraum>.xlsx`). Das Gesamt-Excel behält die Büro-Blätter.
- **Rechte** (5): Der Bauführer sieht unter `/export` nur noch das SORBA-Raster (Titel und Nav «SORBA-Raster»); Lohnstunden,
  Überstunden, Temporärbüros und das Lohn-Excel gibt es nur in der Ansicht Sekretariat (`lohnSichtbar` in Export.tsx).
  Weiterhin nur Oberflächen-Recht (s. «Ansichten statt Login»). **Offen:** sieht der Bauführer alle Baustellen oder nur seine.
- **iPad** (6): Büro-Ansichten bekommen ab `md` (iPad hochkant) eine quer scrollbare Bereichsleiste unter der Kopfzeile und
  eine breitere Spalte (`md:max-w-3xl`); ab `lg` (iPad quer, PC) wie bisher die Seitenleiste. Startseiten-Kacheln und
  Diagramme ab `md`. Handy-Karten der Startseite nur noch unter `md`.
...
src/seiten/Erfassung.tsx          Teamgerät: Kacheln, Anwesenheit (+ Gäste), Stundenzahl oder Zeiten von–bis, Sprachnotiz als Bemerkung zum Tag
...
src/lib/zeiten.ts                 Zeiten von–bis: Uhrzeit ↔ Minuten, Spannen summieren (ohne Pausenrechnung), Normal/Über aufteilen + Tests
...
supabase/migrations/              0001 Schema · 0002 Bezeichnung · 0003 Auftrag-UUID · 0004 Versand · 0005 Stammdaten · 0006 Auftrag-Stand (Sicht) · 0007 Rapport-Ursprung · 0008 Fotos · 0009 Verbesserungen (Indizes, RPCs zeit_freigeben/zeit_korrigieren/zeit_team_setzen/regierapport_anlegen, Nummer RR-JJJJ-NNNN, Sichten regie_kennzahlen/regie_auswertung, Fristen-Job, Storage ohne Delete) · 0010 Beschrieb · 0011 Mehrkostenanzeige · 0012 Rapport-PDF · 0013 Keine Regie · 0014 Kunde weitere E-Mails · 0015 Auftrag-Stand bei Überstunden · 0016 Zeiten von–bis (zeiteintrag.von_min/bis_min/von2_min/bis2_min)
```
