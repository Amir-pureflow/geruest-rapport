...
export function Kachel({ zu, wert, label, warn }: { zu: string; wert: string; label: string; warn?: boolean }) {
  return (
    <Link to={zu} className={'card block hover:border-line-strong ' + (warn ? 'border-amber/50' : '')}>
      <span className={'block font-display text-2xl font-extrabold tabular-nums lg:text-3xl ' + (warn ? 'text-amber-deep' : '')}>{wert}</span>
