...
interface Kunde { id?: string; name: string; ansprechperson: string | null; email: string | null; telefon: string | null; praeferenz: 'einzel' | 'sammel' }

const LEER: Kunde = { name: '', ansprechperson: '', email: '', telefon: '', praeferenz: 'einzel' };
...
export function Kunden() {
  ...
  const laden = useCallback(async () => {
    ...
    const [k, b] = await Promise.all([
      supabase.from('kunde').select('id,name,ansprechperson,email,telefon,praeferenz').order('name'),
      supabase.from('baustelle').select('kunde_id').not('kunde_id', 'is', null),
    ]);
    if (k.error) { setFehler(k.error.message.includes('ansprechperson') ? 'Migration 0005 fehlt — bitte im SQL-Editor ausführen.' : k.error.message); return; }
  ...
  async function speichern() {
    ...
    setFehler('');
    const row = { ...bearbeitet, name: bearbeitet.name.trim(), email: bearbeitet.email?.trim() || null, ansprechperson: bearbeitet.ansprechperson?.trim() || null, telefon: bearbeitet.telefon?.trim() || null };
